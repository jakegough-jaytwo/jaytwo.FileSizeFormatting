...
DOCKER_TAG?=jaytwo_filesizeformatting
...
build: restore
	dotnet build ./jaytwo.FileSizeFormatting.sln
...
unit-test: build
	...
	rm -rf out/coverage
	cd ./test/jaytwo.FileSizeFormatting.Tests; \
		dotnet test \
		--results-directory ../../out/testResults \
		--logger "trx;LogFileName=jaytwo.FileSizeFormatting.Tests.trx"
	reportgenerator \
...
pack:
	rm -rf out/packed
	cd ./src/jaytwo.FileSizeFormatting; \
